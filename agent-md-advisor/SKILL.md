...
description: AGENTS.md 最佳实践顾问。用于用户询问 AGENTS.md 指令文件的格式、结构、最佳实践；也用于审查、诊断、重写、优化或从零创建 AGENTS.md。
---

# AGENTS.md Advisor

帮助用户回答、审查、优化和创建 `AGENTS.md`。

## 设计模式

本 skill 主要采用：
- **Advisor**：回答格式、结构、放什么/不放什么等最佳实践问题
- **Reviewer**：诊断已有 `AGENTS.md` 是否违反最佳实践
- **Generator**：根据项目和用户描述创建新的指令文件
- **Editor**：用户明确要求执行优化时，直接修改相关文件

## Gotchas

- 只处理 `AGENTS.md`；遇到非 `AGENTS.md` 指令文件时，明确说明不在本 skill 范围内
- 不要把 `AGENTS.md` 写成项目百科、README 复制品或完整 API 文档
- 不要把 `AGENTS.md` 写成人设说明书、口号墙或愿望清单；它应该像给资深工程师的技术简报
...
- 不要默认建议复杂 hooks、MCP、skills；先判断项目是否真的需要
- 如果用户只要求诊断，不要直接改文件；如果用户说“优化、重写、创建、执行”，可以落地修改
- 保持文件短而可执行：`AGENTS.md` 理想上 150 行以内，超过 200 行要强烈考虑拆分

## 工作模式

先判断用户需求属于哪一类：

1. **问答模式**：解释 `AGENTS.md` 格式、章节、层级和最佳实践。
2. **诊断模式**：用户已有文件，希望指出问题、违反的实践和改进建议。
3. **优化模式**：用户明确要求修改、重写、压缩、整理或“执行优化”。
4. **创建模式**：用户还没有文件，或希望根据描述生成 `AGENTS.md`。
...
- 本 skill 的唯一目标文件是 `AGENTS.md`
- 跨工具、Codex、Copilot、Cursor、通用 AI coding agent：使用 `AGENTS.md`
- 每个项目都会重复的工作偏好：优先放 global/user 级配置
- 当前项目、技术栈、架构和命令：优先放 repo 内 `AGENTS.md`
- 个人本地偏好：不要提交到团队共享 `AGENTS.md`
- Monorepo 或多技术栈：考虑根目录文件 + 子目录嵌套文件
- 用户要求处理非 `AGENTS.md` 指令文件时，明确说明本 skill 不处理

如果用户没有指定文件名：

- 想要通用标准：建议 `AGENTS.md`
- 同时使用多种工具：建议以 `AGENTS.md` 为主
...
- 现有 AI 配置：`.cursor/rules/`、`.github/copilot-instructions.md`
