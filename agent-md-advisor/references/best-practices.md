...
`AGENTS.md` is a context injection file for AI coding agents. It is not a README replacement, API manual, style encyclopedia, generic “be careful” reminder, persona prompt, or motivational poster.

Use it to tell an otherwise stateless coding agent:
...
Do not put personal local preferences into a team-shared `AGENTS.md`. Keep the file focused on stable project facts and constraints that help most agents in most sessions.
...
- tool-specific rule systems only when path-scoped loading is useful and already part of the project
